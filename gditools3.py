...
"""
    ...
    Converted to Python 2.x & 3.x by Ricardo Mendonça Ferreira, 2018
...
class CdImage():
    ...
    def __init__(self, filename, mode = 'auto', manualRawOffset=0, *args, **kwargs):
        ...
        elif mode not in [2048, 2352, 2336]:
        ...
        if self.__mode in [2352, 2336]:
              self.length = self.file.tell() * 2048//self.__mode
        else: self.length = self.file.tell()
        self.file.seek(0,0)
    ...
    def realOffset(self, a):
        return int(a//2048*self.__mode + a%2048 + self._sectorOffset + self.__manualRawOffset)
    ...
    def read(self, length = None):
        if self.__mode == 2048:
            ...
            buff = BytesIO(self.file.read(realLength))
